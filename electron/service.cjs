...
class CoachService extends EventEmitter {
  ...
  loadCatalog(){const read=(f,def)=>{try{return JSON.parse(fs.readFileSync(path.join(this.dataRoot,f),'utf8').replace(/^\uFEFF/,''))}catch{return def}};this.problems=read('problems.json',[]);this.cases=read('cases.json',{});for(const p of Object.values(this.store.all('problem'))){const i=this.problems.findIndex(x=>x.id===p.id);if(i<0)this.problems.push(p);else this.problems[i]=p}Object.assign(this.cases,this.store.all('cases'))}
  ...
  async detectRuntime(){try{return await this.getRunner().detect()}catch(e){return {python:{available:false},cpp:{available:false},error:e.message,isolation:'运行器不可用'}}}
  ...
  validateInput(p){this.problem(p.problemId);if(!['python','cpp'].includes(p.language))throw new Error('不支持的编程语言');if(typeof p.code!=='string'||p.code.length>200000)throw new Error('代码为空或超过 200 KB');if(p.cases&&(!Array.isArray(p.cases)||p.cases.length>100||JSON.stringify(p.cases).length>1000000))throw new Error('自定义测试过大');}
  async run(p){this.validateInput(p);if(this.running)throw new Error('已有测试正在运行，请先停止');this.running=true;try{const cases=p.cases??this.cases[p.problemId]??[];if(!cases.length)throw new Error('此题尚无已核验用例，请导入或添加自定义用例');const result=await this.getRunner().run({problem:this.problem(p.problemId),language:p.language,code:p.code,cases});this.lastTests.set(p.problemId,{...result,code:p.code,language:p.language});return result}finally{this.running=false}}
  async review(p){this.validateInput(p);if(this.active.has(p.problemId))throw new Error('本题正在生成回答，请先停止或等待');const tests=await this.run(p);const sub=this.store.add('submission',{problemId:p.problemId,language:p.language,code:p.code,tests,review:'',status:'pending',model:p.model||null});this.event({type:'dataChanged',problemId:p.problemId});try{return {...await this.startAI(p,'请批改本次提交。用中文提供总体判断、具体错误及行号、失败原因、可复现反例、时间/空间复杂度、题目规模适配性和最小必要修改建议。区分已执行测试和推理反例；本地通过不等于正确性证明。',sub),submissionId:sub.id,tests}}catch(e){this.store.put('submission',sub.id,{...sub,status:'failed',error:e.message});this.event({type:'dataChanged',problemId:p.problemId});throw e}}
  ...
  async startAI(p,message,sub){
    ...
    try{
      ...
      const old=this.store.get('thread',p.problemId);const params={model:a.model,modelProvider:'openai',cwd:this.client.cwd,approvalPolicy:'never',sandbox:'read-only',baseInstructions:COACH_INSTRUCTIONS,developerInstructions:COACH_INSTRUCTIONS};
      let thread=old?.id;
      ...
      a.threadId=thread;
      const problem=this.problem(p.problemId);const currentTests=sub?.tests||(this.lastTests.get(p.problemId)?.code===p.code?this.lastTests.get(p.problemId):null);
      const context={problem:{id:problem.id,title:problem.title,content:problem.content,meta:problem.meta,url:problem.url},language:p.language,codeSnapshot:p.code,selectedCode:p.selectedCode||'',tests:currentTests,submissionId:sub?.id||null};
  ...
  async invoke(method,p={}){switch(method){
    ...
    case 'settings':{const old=this.store.get('settings','app',{});const safe={};for(const k of ['theme','fontSize','language','model','currentProblem','lastProblemId','chatWidth','sidebarCollapsed'])if(Object.hasOwn(p.patch||{},k))safe[k]=p.patch[k];return this.store.put('settings','app',{...old,...safe})}
    case 'history':return this.store.history(p.problemId);
    case 'run':return this.run(p);case 'stopRun':return this.runner?.stop();case 'review':return this.review(p);case 'chat':return this.chat(p);case 'stopAI':return this.stopAI(p.problemId);
    case 'format':this.validateInput(p);return this.getRunner().format(p);
  ...
  close(){this.runner?.stop();for(const a of [...this.active.values()])this.finish(a,'interrupted','应用已关闭，生成已停止');this.client.close()}
}
